...
**Lacunas conhecidas do template — verifique antes de escrever "Feito quando":** não existe script de `lint` (#8) nem suíte de testes utilizável (#19), e a migração para Vite (#18) vai trocar os comandos de verificação. Não escreva critério de aceite que dependa de um comando que o `package.json` ainda não tem: confira o `package.json` real da branch antes. Enquanto #19 não estiver pronta, critério de tarefa de comportamento é validação manual objetiva.
